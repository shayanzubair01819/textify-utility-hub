...
export const ArticleFormattingTool = () => {
  ...
  const updateTextStats = useCallback((value: string) => {
    ...
    if (wordCount > 0) {
      ...
      Object.entries(wordFrequency).filter(([word, count]) => count > 1).sort(([wordA, countA], [wordB, countB]) => countB - countA).slice(0, 10)
      .forEach(([word, count]) => {
...
<body>
  ...
  const convertToHTML = () => {
    let htmlText = text.replace(/#{6}\s?([^\n]+)/g, '<h6>$1</h6>').replace(/#{5}\s?([^\n]+)/g, '<h5>$1</h5>').replace(/#{4}\s?([^\n]+)/g, '<h4>$1</h4>').replace(/#{3}\s?([^\n]+)/g, '<h3>$1</h3>').replace(/#{2}\s?([^\n]+)/g, '<h2>$1</h2>').replace(/#{1}\s?([^\n]+)/g, '<h1>$1</h2>').replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>').replace(/\*([^*]+)\*/g, '<em>$1</em>').replace(/~~([^~]+)~~/g, '<s>$1</s>').replace(/`([^`]+)`/g, '<code>$1</code>').replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2">$1</a>').replace(/^\s*>\s*(.+)/gm, '<blockquote>$1</blockquote>').replace(/^\s*-\s*(.+)/gm, '<li>$1</li>').replace(/^\s*\d+\.\s*(.+)/gm, '<li>$1</li>');
